 ...
 * @return { void }
 */
var checkPropsChange = function checkPropsChange(props, check) {
  return props && Object.keys(props).map(function (key) {
    if (props[key] !== check[key]) throw new Error("Props should not be changed when rendering a domNode!");
    if (_typeof(props[key]) === 'object' && _constants.default.Schema.PROPS_CHECK.indexOf(key) === -1) checkPropsChange(props[key], check[key]);
  });
};
/**
...
var addSchemaComponent = function addSchemaComponent(schema, id) {
  return (0, _object_util.addProp)(schema, 'domNode', {
    get: function get() {
...
var buildSchema = function buildSchema(curSchema, type, settings) {
  var schema = _objectSpread({}, curSchema, {
...
var loopSource = function loopSource(curSchema, tree, settings, elementCb) {
  ...
  var isRoot = key === _constants.default.Schema.ROOT;
  var cutMode = mode === _constants.default.Schema.MODES.CUT; // pending gets set when empty value is added, and the type was updated
  ...
  var matchTypes = !cutMode && !pending && Types.getValueTypes(value, settings); // Get the type based on the found types, or the current type
  // If pending is set, but use the current type on the schema

  var type = pending ? !cutMode && Types.get(curSchema.matchType) : !cutMode && (0, _match_util.checkMultiMatches)(matchTypes, curSchema, tree, settings); // Check if the type has a factory to call, if not just return
  ...
  var schema = buildSchema(curSchema, type, settings); // If not the root element, set the parent to the schema
  ...
  var props = {
    schema: schema,
    tree: tree,
    settings: settings // Check if there is a shouldUpdate method, and is so call it

  };
  var shouldUpdate = (0, _jsutils.checkCall)(schema.instance.shouldComponentUpdate, curSchema, props);
  ...
  var domNode = (0, _instance_util.renderInstance)(key, value, props, loopSource); // Use the id to set the domNode prop on the schema
...
var appendTreeHelper = function appendTreeHelper(jtree, rootComp, appendTree, tree) {
  var res = (0, _jsutils.checkCall)(appendTree, rootComp, jtree, tree);
...
var buildFromPos = function buildFromPos(jtree, pos, settings) {
  if (!(0, _jsutils.isStr)(pos) || !jtree.tree.schema[pos]) return logData("Rebuild was called, but ".concat(pos, " does not exist it the tree"), jtree.tree, pos, 'warn');
  var renderSchema = jtree.tree.schema[pos];
  var updatedEl = loopSource(renderSchema, jtree.tree, settings, appendTreeHelper); // If updated element was not returned,
