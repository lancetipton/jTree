...
const addProp = (obj, name, def) => jsutils.isObj(obj) && Object.defineProperty(obj, name, def);
...
let useValues = jsutils.deepFreeze({ ...Values
});
const updateDefValues = update => useValues = jsutils.deepFreeze({ ...useValues,
  ...update
});
let useSchema = jsutils.deepFreeze({ ...Schema
});
const updateDefSchema = update => useSchema = jsutils.deepFreeze({ ...useSchema,
  ...update
});
const Constants = {
  updateDefSchema,
...
const validateMatchType = (checkType, TYPE_CACHE) => {
  ...
  if (failedClsProps.length) return jsutils.logData(`Could not register type '${checkType.name || 'Type'}'. It's missing these static properties:\n\t${failedClsProps.join('\n\t')}`, 'error');
  if (TYPE_CACHE && TYPE_CACHE[checkType.name]) return jsutils.logData(`Type with name ${checkType.name} is already registered!`, 'error');
...
const buildStyleId = Type => {
  Type.styleId = `${Type.name.toLowerCase()}-${jsutils.uuid().split('-').pop()}`;
  return Type.styleId;
};
const initTypeCache = (TypesCls, settings) => {
  const {
    BaseType,
    ...allTypes
  } = jsutils.get(settings.types, 'definitions') || {};
...
const getExtends = factory => {
  const parent = factory.__proto__ && jsutils.get(factory.__proto__, 'prototype.constructor');
  return parent && {
    name: parent.name,
    base: parent,
...
const buildTypeCache = (settings, types) => {
  ...
  Object.defineProperty(builtTypes, Constants.Values.MAP_TYPES, {
    value: (cb, parent) => jsutils.mapObj(parent, cb),
    enumerable: false
  });
...
const buildInstance = (type, schema, settings) => {
  ...
  if (!INSTANCE_CACHE[id]) {
    const config = jsutils.get(settings.types, `config.${matchType}`) || {};
    const editorConfig = settings.Editor.config || {};
    jsutils.mapObj(Constants.Values.CUSTOM_EVENTS, (key, value) => !config[key] && editorConfig[key] && (config[key] = editorConfig[key]));
    ...
    Object.keys(instance).map(key => {
      ...
      instance[key] = (...args) => {
        ...
        let callOrg = false;
        let hasOverride = jsutils.isFunc(editorConfig[key]);
        if (hasOverride) {
...
const renderInstance = (key, value, props, loopChildren) => {
  ...
  let domNode = jsutils.isObj(value) ? jsutils.checkCall(schema.instance.render, { ...props,
    children: Object.entries(value).map(([childKey, child]) => buildChild(childKey, child, props, loopChildren))
  }) : Array.isArray(value) ? jsutils.checkCall(schema.instance.render, { ...props,
...
const clearTypeCache = typeCache => {
  jsutils.isObj(typeCache) && Object.keys(typeCache).map(key => {
    switch (key) {
      case 'children':
        Object.values(typeCache).map(child => clearTypeCache(child));
        break;
      case 'extends':
        Object.keys(typeCache.extends).map(key => jsutils.unset(typeCache.extends, key));
        break;
    }
...
const clearSchema = (schema, tree, removeInstance = true) => {
  ...
  jsutils.mapObj(tree.schema, (pos, schemaItem) => {
...
const checkConfirm = (...params) => {
  ...
  const outcome = jsutils.isFunc(CUSTOM_CONFIRM) ? CUSTOM_CONFIRM(...params) : window.confirm(message);
...
const checkMultiMatches = (matchTypes, schema, tree, settings) => {
  let hasMatches = jsutils.isObj(matchTypes);
...
const loopSource = (curSchema, tree, settings, elementCb) => {
  ...
  const shouldUpdate = jsutils.checkCall(schema.instance.shouldComponentUpdate, curSchema, props);
...
const appendTreeHelper = (jtree, rootComp, appendTree, tree) => {
  const res = jsutils.checkCall(appendTree, rootComp, jtree, tree);
...
const buildFromPos = (jtree, pos, settings) => {
  if (!jsutils.isStr(pos) || !jtree.tree.schema[pos]) return logData(`Rebuild was called, but ${pos} does not exist it the tree`, jtree.tree, pos, 'warn');
...
const checkSchemaPos = (tree, pos, checkExists) => checkExists ? !tree.schema[pos] ? jsutils.logData(`Cannot update schema in tree. Schema does not exist!`, pos, tree.schema[pos], 'error') : true : tree.schema[pos] ? jsutils.logData(`Cannot add child to tree. Schema pos already exists!`, pos, tree.schema[pos], 'error') : true;
...
const updateSchema = (update, schema) => {
  return jsutils.isObj(update) && jsutils.isObj(schema) && Object.entries(update).reduce((updated, [key, value]) => {
...
const updateKey = (tree, pos, schema, settings) => {
  ...
  const currentVal = jsutils.get(tree, pos);
  ...
  const unsetContent = jsutils.unset(tree, pos);
...
const addChildSchema = (tree, schema, parent) => {
  ...
  const parentVal = jsutils.get(tree, parent.pos);
  ...
  class Types {
    constructor() {
      ...
      this.clear = (includeClass = true) => {
        ...
        jsutils.mapObj(FLAT_TYPES, key => jsutils.unset(FLAT_TYPES[key]));
      ...
      this.getValueTypes = value => {
        ...
        const firstKey = jsutils.isObj(matchTypes) && Object.keys(matchTypes)[0];
...
const handelUpdateError = (jTree, pos, settings, prop, value, message) => {
  if (!pos || !jTree.tree.schema[pos]) return jsutils.logData(`Could not find ${pos} in the tree!`);
  ...
  class jTree {
    constructor() {
      this.buildTypes = source => {
        ...
        if (jsutils.isObj(ACT_SOURCE)) this.tree = buildTypes(ACT_SOURCE, settings, appendTreeHelper);
        return this;
      };
      ...
      this.update = (idOrPos, update) => {
        ...
        if (shouldShowConfirm(update) && !checkConfirm(validData.schema, pos, update, `${update.mode && jsutils.capitalize(update.mode) || 'Update'} node at ${pos}?`)) return;
        ...
        this.tree.schema[pos].pending && !update.matchType && jsutils.unset(this.tree.schema[pos], 'pending');
      ...
      this.remove = idOrPos => {
        ...
        jsutils.unset(this.tree, pos);
        jsutils.unset(this.tree.idMap, schema.id);
      ...
      this.add = (schema, parent) => {
        ...
        if (!isValid || isValid.error) return jsutils.logData(isValid.error, schema, parent, this.tree, 'warn');
      ...
      this.schema = idOrPos => jsutils.get(this, ['tree', 'schema', jsutils.get(this, `tree.idMap.${idOrPos}`, idOrPos)]);
      this.destroy = () => {
        ACT_SOURCE = undefined;
        const rootNode = jsutils.get(this, `tree.schema.${Constants.Schema.ROOT}.domNode`);
        jsutils.clearObj(this.tree[Constants.Schema.ROOT]);
        jsutils.clearObj(this.tree.idMap);
        jsutils.checkCall(jsutils.get(this, 'Types.destroy'), this);
        jsutils.clearObj(this.tree);
        jsutils.clearObj(this.config);
        jsutils.unset(this, 'Types');
        jsutils.unset(this, 'element');
        jsutils.unset(this, 'temp');
        cleanUp(settings, this.tree);
        jsutils.clearObj(this);
        if (!rootNode || !rootNode.parentNode) return;
        const classList = jsutils.get(rootNode, 'parentNode.classList');
...
const init = async opts => {
  ...
  if (!domContainer) return jsutils.logData(`Dom node ( element ) is required when calling the jTree init method`, 'error');
  const {
    element,
    showLogs,
    editor,
  ...
  const settings = jsutils.deepMerge(DEF_SETTINGS, options);
  const editorConfig = jsutils.deepMerge(Constants.EditorConfig, editor);
