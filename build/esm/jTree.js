...
var addProp = function addProp(obj, name, def) {
  return isObj(obj) && Object.defineProperty(obj, name, def);
};
...
var useValues = deepFreeze(objectSpread({}, Values));
var updateDefValues = function updateDefValues(update) {
  return useValues = deepFreeze(objectSpread({}, useValues, update));
};
var useSchema = deepFreeze(objectSpread({}, Schema));
var updateDefSchema = function updateDefSchema(update) {
  return useSchema = deepFreeze(objectSpread({}, useSchema, update));
};
var Constants = {
...
var validateMatchType = function validateMatchType(checkType, TYPE_CACHE) {
  ...
  if (failedClsProps.length) return logData$1("Could not register type '".concat(checkType.name || 'Type', "'. It's missing these static properties:\n\t").concat(failedClsProps.join('\n\t')), 'error');
  if (TYPE_CACHE && TYPE_CACHE[checkType.name]) return logData$1("Type with name ".concat(checkType.name, " is already registered!"), 'error');
...
var buildStyleId = function buildStyleId(Type) {
  Type.styleId = "".concat(Type.name.toLowerCase(), "-").concat(uuid().split('-').pop());
  return Type.styleId;
};
var initTypeCache = function initTypeCache(TypesCls, settings) {
  var _ref = get(settings.types, 'definitions') || {},
...
var getExtends = function getExtends(factory) {
  var parent = factory.__proto__ && get(factory.__proto__, 'prototype.constructor');
  return parent && {
    name: parent.name,
    base: parent,
...
var buildInstance = function buildInstance(type, schema, settings) {
  ...
  if (!INSTANCE_CACHE[id]) {
    var config = get(settings.types, "config.".concat(matchType)) || {};
    var editorConfig = settings.Editor.config || {};
    mapObj(Constants.Values.CUSTOM_EVENTS, function (key, value) {
    ...
    Object.keys(instance).map(function (key) {
      ...
      instance[key] = function () {
        ...
        var callOrg = false;
        var hasOverride = isFunc(editorConfig[key]);
        if (hasOverride) {
...
var renderInstance = function renderInstance(key, value, props, loopChildren) {
  ...
  var domNode = isObj(value) ? checkCall(schema.instance.render, objectSpread({}, props, {
  ...
  })) : Array.isArray(value) ? checkCall(schema.instance.render, objectSpread({}, props, {
...
var clearTypeCache = function clearTypeCache(typeCache) {
  return isObj(typeCache) && Object.keys(typeCache).map(function (key) {
    switch (key) {
      case 'children':
      ...
      case 'extends':
        Object.keys(typeCache.extends).map(function (key) {
          return unset(typeCache.extends, key);
        });
        break;
    }
    unset(typeCache, key);
...
var clearSchema = function clearSchema(schema, tree) {
  ...
  clearObj(schema);
  mapObj(tree.schema, function (pos, schemaItem) {
...
var checkConfirm = function checkConfirm() {
  ...
  var outcome = isFunc(CUSTOM_CONFIRM) ? CUSTOM_CONFIRM.apply(void 0, params) : window.confirm(message);
...
var checkMultiMatches = function checkMultiMatches(matchTypes, schema, tree, settings) {
  var hasMatches = isObj(matchTypes);
...
var loopSource = function loopSource(curSchema, tree, settings, elementCb) {
  ...
  var shouldUpdate = checkCall(schema.instance.shouldComponentUpdate, curSchema, props);
...
var appendTreeHelper = function appendTreeHelper(jtree, rootComp, appendTree, tree) {
  var res = checkCall(appendTree, rootComp, jtree, tree);
...
var buildFromPos = function buildFromPos(jtree, pos, settings) {
  if (!isStr(pos) || !jtree.tree.schema[pos]) return logData("Rebuild was called, but ".concat(pos, " does not exist it the tree"), jtree.tree, pos, 'warn');
...
var checkSchemaPos = function checkSchemaPos(tree, pos, checkExists) {
  return checkExists ? !tree.schema[pos] ? logData$1("Cannot update schema in tree. Schema does not exist!", pos, tree.schema[pos], 'error') : true : tree.schema[pos] ? logData$1("Cannot add child to tree. Schema pos already exists!", pos, tree.schema[pos], 'error') : true;
};
...
var updateSchema = function updateSchema(update, schema) {
  return isObj(update) && isObj(schema) && Object.entries(update).reduce(function (updated, _ref) {
    var _ref2 = slicedToArray(_ref, 2),
        key = _ref2[0],
        value = _ref2[1];
...
var updateKey = function updateKey(tree, pos, schema, settings) {
  ...
  var unsetContent = unset(tree, pos);
...
var addChildSchema = function addChildSchema(tree, schema, parent) {
  ...
  var parentVal = get(tree, parent.pos);
...
function TypesCls(settings) {
  var Types = function Types() {
    ...
    defineProperty(this, "clear", function () {
      ...
      mapObj(FLAT_TYPES, function (key) {
        return unset(FLAT_TYPES[key]);
      });
    ...
    defineProperty(this, "getValueTypes", function (value) {
      ...
      var firstKey = isObj(matchTypes) && Object.keys(matchTypes)[0];
...
var handelUpdateError = function handelUpdateError(jTree, pos, settings, prop, value, message) {
  if (!pos || !jTree.tree.schema[pos]) return logData$1("Could not find ".concat(pos, " in the tree!"));
          ...
          case 0:
            jTree = function jTree() {
              ...
              defineProperty(this, "buildTypes", function (source) {
                ...
                if (isObj(ACT_SOURCE)) _this.tree = buildTypes(ACT_SOURCE, settings, appendTreeHelper);
                return _this;
              });
              ...
              defineProperty(this, "update", function (idOrPos, update) {
                ...
                if (shouldShowConfirm(update) && !checkConfirm(validData.schema, pos, update, "".concat(update.mode && capitalize(update.mode) || 'Update', " node at ").concat(pos, "?"))) return;
                ...
                _this.tree.schema[pos].pending && !update.matchType && unset(_this.tree.schema[pos], 'pending');
              ...
              defineProperty(this, "remove", function (idOrPos) {
                ...
                unset(_this.tree, pos);
                unset(_this.tree.idMap, schema.id);
              ...
              defineProperty(this, "add", function (schema, parent) {
                ...
                if (!isValid || isValid.error) return logData$1(isValid.error, schema, parent, _this.tree, 'warn');
              ...
              defineProperty(this, "schema", function (idOrPos) {
                return get(_this, ['tree', 'schema', get(_this, "tree.idMap.".concat(idOrPos), idOrPos)]);
              });
              defineProperty(this, "destroy", function () {
                ACT_SOURCE = undefined;
                var rootNode = _this.tree.schema[Constants.Schema.ROOT].domNode;
                clearObj(_this.tree[Constants.Schema.ROOT]);
                clearObj(_this.tree.idMap);
                clearObj(_this.config);
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            ...
            return _context2.abrupt("return", logData$1("Dom node ( element ) is required when calling the jTree init method", 'error'));
          case 4:
