...
const headerFilter = ['editor-api', 'custom-tools', 'dependencies'];

const addNavItem = (navList, element) => {
  const linkWrp = document.createElement('li');
  const link = document.createElement('a');
  const cleaned = element.innerText.toLowerCase().replace(/ /g, '-');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  navList && Array.from(compHW.getElementsByTagName('h2')).map(element => {
