...
const updateDefSchema = _constants.default.updateDefSchema,
...
setTimeout(() => {
  ...
  const matchHelper = ({
    ...
    return typeof value === 'object' ? AllTypes.map : AllTypes.string;
  };

  const numOnChange = (event, update, id, Editor) => {// console.log('------------------ Num onChange ------------------');
  };

  const init = () => {
    jsonApiCall().then(testData => {
