  ...
  class jTree {
    constructor() {
      _defineProperty(this, "buildTypes", source => {
        ...
        if ((0, _jsutils.isObj)(ACT_SOURCE)) this.tree = (0, _types.buildTypes)(ACT_SOURCE, settings, _jTUtils.appendTreeHelper);
        return this;
      });
      ...
      _defineProperty(this, "forceUpdate", pos => {
        pos && (0, _jTUtils.buildFromPos)(this, pos, settings);
      });
      ...
      _defineProperty(this, "update", (idOrPos, update) => {
        ...
        const validData = (0, _jTUtils.validateUpdate)(this.tree, idOrPos, update, settings); // And Ensure we have a schema, pos to use and there is no error
        ...
        if (shouldShowConfirm(update) && !(0, _jTUtils.checkConfirm)(validData.schema, pos, update, `${update.mode && (0, _jsutils.capitalize)(update.mode) || 'Update'} node at ${pos}?`)) return; // Remove the current error, if one exists
        ...
        let schema = (0, _jTUtils.updateSchema)(update, _objectSpread({}, validData.schema)); // Check for an update to the key and handel it
        ...
        (0, _jTUtils.buildFromPos)(this, pos, settings);
      });

      _defineProperty(this, "replaceAtPos", (idOrPos, replace) => {
        // Ensure the passed in replace object is valid
        const validData = (0, _jTUtils.validateUpdate)(this.tree, idOrPos, {
        ...
        if (!(0, _jTUtils.checkConfirm)(schema, pos, replace, `Replace ${schema.pos}?`)) return; // Update the replace object to include the original schemas location data
      ...
      _defineProperty(this, "remove", idOrPos => {
        ...
        const pos = validData.pos,
              schema = validData.schema;
        if (!(0, _jTUtils.checkConfirm)(schema, pos, `Remove ${pos}?`)) return; // Clear the data from the tree
        ...
        (0, _jTUtils.clearSchema)(schema, this.tree); // Re-render from the parentPos

        (0, _jTUtils.buildFromPos)(this, parentPos, settings);
      });

      _defineProperty(this, "add", (schema, parent) => {
        const useParent = schema.parent || parent || this.tree.schema; // Validate the passed in data

        const isValid = (0, _jTUtils.validateAdd)(schema, useParent);
        if (!isValid || isValid.error) return (0, _jsutils.logData)(isValid.error, schema, parent, this.tree, 'warn');
        if (schema.matchType !== _constants.default.Schema.EMPTY && !(0, _jTUtils.checkConfirm)(schema, useParent.pos, `Add to parent ${useParent.pos}?`)) return; // Add the child schema to the parent / tree

        if (!(0, _jTUtils.addChildSchema)(this.tree, schema, useParent)) return; // Rebuild the tree from parent position

        (0, _jTUtils.buildFromPos)(this, useParent.pos, settings);
      });
      ...
      _defineProperty(this, "destroy", () => {
        ...
        (0, _jTUtils.cleanUp)(settings, this.tree);
